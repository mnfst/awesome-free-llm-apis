/**
 * @file openrouter-smoke-test.ts
 * @description Verifies models added/removed for the OpenRouter provider by the
 * 2026-07-30 upstream data.json merge. Usage: tsx scripts/verification/providers/openrouter-smoke-test.ts
 */
...
async function testModel(provider: OpenRouterProvider, id: string) {
...
async function runOpenRouterSmokeTest() {
    ...
    const openrouter = registry.getProvider('openrouter') as OpenRouterProvider;

    if (!openrouter) {
        console.error('OpenRouter provider not found in registry.');
        process.exit(1);
    }

    if (!openrouter.isAvailable()) {
        console.error('OpenRouter provider is not available (check OPENROUTER_API_KEY).');
        process.exit(1);
    }

    const { added, removed } = CHANGED_MODELS.openrouter;

    console.error(`\n=== OpenRouter Changed-Model Smoke Test ===`);
    console.error(`Added models: ${added.length} | Removed models: ${removed.length}\n`);

    console.error(`--- Added models ---\n`);
    const addedResults = [];
    for (const id of added) addedResults.push(await testModel(openrouter, id));

    console.error(`--- Removed models (checking if they still work) ---\n`);
    const removedResults = [];
    for (const id of removed) removedResults.push(await testModel(openrouter, id));

    console.log(`\n=== Results Summary: OpenRouter ===`);
    console.log(`\nAdded models:`);
...
runOpenRouterSmokeTest().catch(error => {
    console.error('Fatal error during OpenRouter smoke test:', error);
    process.exit(1);
});
