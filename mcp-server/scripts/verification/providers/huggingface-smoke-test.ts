/**
 * @file huggingface-smoke-test.ts
 * @description Verifies models added/removed for the Hugging Face provider by the
 * 2026-07-30 upstream data.json merge. Usage: tsx scripts/verification/providers/huggingface-smoke-test.ts
 */
...
async function testModel(provider: HuggingFaceProvider, id: string) {
...
async function runHuggingFaceSmokeTest() {
    const registry = ProviderRegistry.getInstance();
    const hf = registry.getProvider('huggingface') as HuggingFaceProvider;

    if (!hf) {
        console.error('Hugging Face provider not found in registry.');
        process.exit(1);
    }

    if (!hf.isAvailable()) {
        console.error('Hugging Face provider is not available (check HF_TOKEN).');
        process.exit(1);
    }

    const { added, removed } = CHANGED_MODELS.huggingface;

    console.error(`\n=== Hugging Face Changed-Model Smoke Test ===`);
    console.error(`Added models: ${added.length} | Removed models: ${removed.length}\n`);

    console.error(`--- Added models ---\n`);
    const addedResults = [];
    for (const id of added) addedResults.push(await testModel(hf, id));

    console.error(`--- Removed models (checking if they still work) ---\n`);
    const removedResults = [];
    for (const id of removed) removedResults.push(await testModel(hf, id));

    console.log(`\n=== Results Summary: Hugging Face ===`);
    console.log(`\nAdded models:`);
...
runHuggingFaceSmokeTest().catch(error => {
    console.error('Fatal error during Hugging Face smoke test:', error);
    process.exit(1);
});
