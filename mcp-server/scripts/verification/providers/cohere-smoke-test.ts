/**
 * @file cohere-smoke-test.ts
 * @description Verifies models added/removed for the Cohere provider by the
 * 2026-07-30 upstream data.json merge. Usage: tsx scripts/verification/providers/cohere-smoke-test.ts
 */
...
async function testModel(provider: CohereProvider, id: string) {
...
async function runCohereSmokeTest() {
    const registry = ProviderRegistry.getInstance();
    const cohere = registry.getProvider('cohere') as CohereProvider;

    if (!cohere) {
        console.error('Cohere provider not found in registry.');
        process.exit(1);
    }

    if (!cohere.isAvailable()) {
        console.error('Cohere provider is not available (check CO_API_KEY).');
        process.exit(1);
    }

    const { added, removed } = CHANGED_MODELS.cohere;

    console.error(`\n=== Cohere Changed-Model Smoke Test ===`);
    console.error(`Added models: ${added.length} | Removed models: ${removed.length}\n`);

    console.error(`--- Added models ---\n`);
    const addedResults = [];
    for (const id of added) addedResults.push(await testModel(cohere, id));

    console.error(`--- Removed models (checking if they still work) ---\n`);
    const removedResults = [];
    for (const id of removed) removedResults.push(await testModel(cohere, id));

    console.log(`\n=== Results Summary: Cohere ===`);
    console.log(`\nAdded models:`);
...
runCohereSmokeTest().catch(error => {
    console.error('Fatal error during Cohere smoke test:', error);
    process.exit(1);
});
