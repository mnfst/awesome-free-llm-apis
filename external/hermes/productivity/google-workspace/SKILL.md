...
- **Email + Calendar** → Continue with this skill. The current setup script
  requests the full Google Workspace scope set, so explain that consent is
  broad even if the user's task is narrow.

- **Calendar/Drive/Sheets/Docs only** → Continue with this skill. The current
  setup script requests the full Google Workspace scope set.
...
an older browser tab, run `$GSETUP --auth-url` again and send the fresh printed
URL to the user.
